...
from src.pipeline.scanner_pipeline import ScannerPipeline  # type: ignore  # noqa: E402
from src.pipeline.poc_pipeline import POCPipeline  # type: ignore  # noqa: E402
from src.database import get_db  # type: ignore  # noqa: E402
from src.database.repository import ScanResultRepository  # type: ignore  # noqa: E402
...
def find_latest_nuclei_scan_id(target_url: str) -> Optional[int]:
    """특정 target_url에 대한 가장 최근 Nuclei 스캔 결과 ID 조회"""
    db = get_db()
    with db.get_session() as session:
        repo = ScanResultRepository(session)
        scans = repo.get_recent(days=1, limit=50)
        for s in scans:
...
def run_text4shell_workflow(target: str) -> None:
    """
    Text4Shell 스캔 + PoC 워크플로우 실행

    Args:
        target: EC2 퍼블릭 IP 또는 호스트네임 (예: 13.125.x.x)
    """
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.warning(f"Nmap 스캔 중 오류(계속 진행): {e}")
    ...
    scan_result_id = find_latest_nuclei_scan_id(target_url)
    if not scan_result_id:
        logger.error("DB에서 Nuclei 스캔 결과를 찾지 못했습니다. PoC를 진행할 수 없습니다.")
        return

    logger.info(f"Using scan_result_id={scan_result_id} for PoC reproduction")
